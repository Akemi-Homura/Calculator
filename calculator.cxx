...
int main()
{
    ...
    cout << "  test Rpn   6:"; try {
        TestInfixToPostfix ("1/(2+3)+4*(5-6/7)*8/9-0;", "123+/4567/-*8*9/+0-"                    ); } catch (const char* message) { cout << "Exception: " << message << endl; return -1; } cout << " success" << endl;
...
string InfixToPostfix(string infix)
{
	...
	CharStack res, operation;
	for (int i = 0; i < (int)infix.size(); i++) {
		char c = infix[i];
		if (c == ';')break;
		if (IsOper(c)){
            if(operation.IsEmpty() || c == '(' || Precedence(operation.Top()) < Precedence(c) ) {
			    operation.Push(c);
                continue;
            }else {
			    while(!operation.IsEmpty() && operation.Top() != '(' && Precedence(operation.Top()) >= Precedence(c))
...
double DoubleOperation(double num1, double num2, char op) {
	double res;
	switch (op) {
	case '+' :
		res = num1 + num2;
...
double EvaluatePostfix(string postfix)
{
	...
	for (int i = 0; i < (int)postfix.size(); i++) {
		...
		if (IsOper(c)) {
			...
			NumStack.Push(DoubleOperation(num1, num2, c));
		}
		else {
			NumStack.Push(CharToDouble(c));
		}
	}
    return NumStack.Pop();
    // Implement function body
